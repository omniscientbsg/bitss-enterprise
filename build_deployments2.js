...
content = content.replace(/<>\\n\\s*<Navbar \/>\\n\\s*<main className=\"flex-1 pt-32 px-6 max-w-7xl mx-auto w-full min-h-\[80vh\]\">/, '<div className=\"bg-void font-body text-text relative min-h-screen overflow-x-hidden selection:bg-accent2/30 selection:text-white\">\\n      <Navbar />\\n      <main className=\"flex-1 pt-[140px] px-6 lg:px-16 max-w-[1200px] mx-auto w-full min-h-[80vh] pb-32\">');
content = content.replace(/<Footer \/>\\n\\s*<\/>/, '<Footer />\\n    </div>');

content = content.replace('text-slate-50 mb-8', 'font-display text-white tracking-tight mb-8 leading-[1.1]');
content = content.replace('text-slate-400 text-lg max-w-3xl leading-relaxed mb-12', 'font-body text-white/60 text-lg max-w-3xl leading-[1.8] font-light mb-16');
